...
def trigger_cloudflare_ingest(clip_id: str) -> None:
    with Session(engine) as db:
        ...
        if resp.status_code != 200:
            log.error("Cloudflare Stream copy ingest failed: %s", resp.text)
            clip.status = "failed"
            db.add(clip)
            db.commit()
            return
        ...
        if not uid:
            log.error("Cloudflare Stream response missing result.uid: %s", resp.text)
            clip.status = "failed"
            db.add(clip)
            db.commit()
            return
        ...
        db.add(clip)
        db.commit()
        log.info("clip %s → stream_uid %s, status=processing", clip_id, uid)
